...
$model_company_child = \common\models\Customer::find()->where(['status'=>1])->all();
...
//                    ?>
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <?= $form->field($model, 'customer_invoice_id')->Widget(\kartik\select2\Select2::className(), [
                'data' => \yii\helpers\ArrayHelper::map(\backend\models\Customer::find()->where(['status' => 1,'is_invoice'=>1])->all(), 'id', function ($data) {
                    return $data->name;
                }),
                'options' => [
                    'placeholder' => '--เลือกลูกค้าวางบิล--'
                ]
            ]) ?>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <table class="table table-bordered table-striped" id="table-customer-list">
                ...
                <tr>
                    <td colspan="3">
                        <div class="btn btn-primary" onclick="addlinecustomer($(this))">
                            <i class="fa fa-plus-circle"></i>
                        </div>
                    </td>
                </tr>
...
function addselecteditem(e) {
        ...
        var customer_id = e.closest('tr').find('.line-find-customer-id').val();
        var customer_name = e.closest('tr').find('.line-find-customer-name').val();
        ///////
        if (id) {
            ...
            if (e.hasClass('btn-outline-success')) {
                ...
                obj['line_customer_name'] = customer_name;
...
$(".btn-emp-selected").click(function () {
        var linenum = 0;
        if(selecteditem.length >0){
             var tr = $("#table-customer-list tbody tr:last");
             ...
             for(var i=0;i<=selecteditem.length-1;i++){
               //  var new_text = selecteditem[i]['line_work_type_name'] + "\\n" + "Order No."+selecteditem[i]['line_order_no'];
                   if (tr.closest("tr").find(".line-customer-id").val() == "") {
                  //  alert(line_prod_code);
            
                    tr.closest("tr").find(".line-customer-id").val(selecteditem[i]['customer_id']);
                    tr.closest("tr").find(".line-customer-name").val(selecteditem[i]['line_customer_name']);
                    //console.log(line_prod_code);
                    } else {
                        var clone = tr.clone();
                        clone.closest("tr").find(".line-rec-id").val('0');
                        clone.closest("tr").find(".line-customer-id").val(selecteditem[i]['customer_id']);
                        clone.closest("tr").find(".line-customer-name").val(selecteditem[i]['line_customer_name']);
