...
$find_month = $find_month==null?'-1':$find_month;
...
$work_queue_type = \yii\helpers\ArrayHelper::map(\backend\helpers\WorkQueueType::asArray(),'id','name');
...
function getLineData2($customer_id, $find_year, $car_type_id, $find_month)
{
    ...
    if ($find_month != '-1' || $find_month != null) {
        $sql .= " AND month(t1.work_queue_date)=" . $find_month;
    }
